...
class EnvWrapper:
    def __init__(self, train_mode: bool = True, env_name: str = None, n_envs: int = 1):
    ...
    def step(
        self, ma_d_action: Dict[str, np.ndarray], ma_c_action: Dict[str, np.ndarray]
    ):
        """
        Args:
...
class UnityWrapperProcess:
    def __init__(
        ...
        if conn:  # If run in multiprocessing mode
            ...
            self._logger = logging.getLogger(
                f"UnityWrapper.Process_{worker_id}_p{os.getpid()}"
            )
        else:
    ...
    def init(self):
        ...
        for n in self.behavior_names:
            ...
            if behavior_spec.action_spec.discrete_size > 0:
                for branch, branch_size in enumerate(
                    behavior_spec.action_spec.discrete_branches
                ):
                    discrete_action_sizes.append(branch_size)
                    self._logger.info(
                        f"{n} Discrete action branch {branch} has {branch_size} different actions"
                    )
            ...
            if self.group_aggregation:
                decision_steps, terminal_steps = self._env.get_steps(n)
                u_group_ids, u_group_id_counts = np.unique(
                    decision_steps.group_id, return_counts=True
                )
                # u_group_id_counts: agent count of each group id
                max_u_group_id_count = (
                    u_group_id_counts.max()
                )  # the max agent count in each group id

                self.ma_obs_shapes[n] = [
                    (max_u_group_id_count, *obs_shape)
                    for obs_shape in self.ma_obs_shapes[n]
                ]
                self.ma_d_action_sizes[n] = (
                    self.ma_d_action_sizes[n] * max_u_group_id_count
                )  # list[int]
                self.ma_c_action_size[n] = (
                    self.ma_c_action_size[n] * max_u_group_id_count
                )  # int

                for branch, branch_size in enumerate(self.ma_d_action_sizes[n]):
                    self._logger.info(
                        f"{n} Aggregated discrete action branch {branch} has {branch_size} different actions"
                    )
                self._logger.info(
                    f"{n} Aggregated continuous action size: {self.ma_c_action_size[n]}"
                )

            for o_name, o_shape in zip(obs_names, obs_shapes):
                if (
                    "camera" in o_name.lower()
                    or "visual" in o_name.lower()
                    or "image" in o_name.lower()
                ) and len(o_shape) >= 3:
                    self.engine_configuration_channel.set_configuration_parameters(
                        quality_level=5
                    )
    ...
    def reset(self, reset_config=None):
        ...
        for n in self.behavior_names:
            ...
            self._ma_agent_id_to_index[n] = {
                a_id: a_idx for a_idx, a_id in enumerate(decision_steps.agent_id)
            }
            ...
            if self.group_aggregation:
                ...
                for i, obs in enumerate(ma_obs_list[n]):
                    aggr_obs = np.zeros(
                        (len(u_group_ids), u_group_id_counts.max(), *obs.shape[1:]),
                        dtype=obs.dtype,
                    )
                    for j, (group_id, group_id_count) in enumerate(
                        zip(u_group_ids, u_group_id_counts)
                    ):
                        aggr_obs[j, :group_id_count] = obs[
                            decision_steps.group_id == group_id
                        ]
    ...
    def step(self, ma_d_action, ma_c_action):
        ...
        if self.group_aggregation:
            for n in self.behavior_names:
                if self.ma_d_action_sizes[n]:
                    ...
                    unity_d_action = np.zeros(
                        (len(self._ma_agents_ids[n]), unity_d_action_summed_size),
                        dtype=ma_d_action[n].dtype,
                    )

                    for action, group_id, group_id_count in zip(
                        d_action, self._ma_u_group_ids[n], self._ma_u_group_id_counts[n]
                    ):
                        unity_d_action[
                            self._ma_group_ids[n] == group_id
                        ] = action.reshape(-1, unity_d_action_summed_size)[
                            :group_id_count
                        ]
                ...
                if self.ma_c_action_size[n]:
                    ...
                    unity_c_action = np.zeros(
                        (len(self._ma_agents_ids[n]), unity_c_action_size),
                        dtype=c_action.dtype,
                    )

                    for action, group_id, group_id_count in zip(
                        c_action, self._ma_u_group_ids[n], self._ma_u_group_id_counts[n]
                    ):
                        unity_c_action[
                            self._ma_group_ids[n] == group_id
                        ] = action.reshape(-1, unity_c_action_size)[:group_id_count]
        ...
        for n in self.behavior_names:  # padding all data
            ...
            ma_obs_list[n] = [
                np.zeros((agents_len, *obs_shape), dtype=np.float32)
                for obs_shape in self.ma_unity_obs_shapes[n]
            ]
        ...
        for n in self.behavior_names:  # sending actions to the environment
            ...
            if self.ma_unity_d_action_sizes[n]:
                d_action_list = np.split(
                    ma_d_action[n], np.cumsum(self.ma_unity_d_action_sizes[n]), axis=-1
                )[:-1]
                d_action_list = [
                    np.argmax(d_action, axis=-1) for d_action in d_action_list
                ]
            ...
            self._env.set_actions(
                n, ActionTuple(continuous=c_action, discrete=d_action)
            )
        ...
        while any(
            [
                ma_decision_steps_len[n] != len(self._ma_agents_ids[n])
                for n in self.behavior_names
            ]
        ):
            for n in self.behavior_names:
...
class UnityWrapper(EnvWrapper):
            ...
            for i in range(self.env_length):
                self._envs.append(
                    UnityWrapperProcess(
                        ...
                        n_envs=min(
                            MAX_N_ENVS_PER_PROCESS,
                            self.n_envs - i * MAX_N_ENVS_PER_PROCESS,
                        ),
    ...
    def _process_conn_receiving(
        self, conn: multiprocessing.connection.Connection, i: int
    ):
        if not conn.poll(60):
        ...
        while None in self._conns:
            ...
            p = multiprocessing.Process(
                ...
                args=(
                    ...
                    min(
                        MAX_N_ENVS_PER_PROCESS, self.n_envs - i * MAX_N_ENVS_PER_PROCESS
                    ),
            ...
            self._logger.info(
                f"Environment {i} started with process {self._process_id}"
            )
            self._process_id += 1
    ...
    def reset(self, reset_config=None):
        ...
        ma_obs_list = {
            n: [
                np.concatenate(env_obs_list)
                for env_obs_list in zip(
                    *[ma_obs_list[n] for ma_obs_list in envs_ma_obs_list]
                )
            ]
            for n in self.behavior_names
        }
    ...
    def step(
        ...
        for i in range(self.env_length):
            ...
            for n in self.behavior_names:
                ...
                if d_action is not None:
                    tmp_ma_d_actions[n] = d_action[
                        n_agents_list[i] : n_agents_list[i + 1]
                    ]
                if c_action is not None:
                    tmp_ma_c_actions[n] = c_action[
                        n_agents_list[i] : n_agents_list[i + 1]
                    ]
            ...
            if self._seq_envs:
                (
                    ...
                    ma_max_step,
                    ma_padding_mask,
                ) = self._envs[i].step(tmp_ma_d_actions, tmp_ma_c_actions)
        ...
        if not self._seq_envs:
            ...
            for i, conn in enumerate(self._conns):
                try:
                    ...
                    self._logger.error(
                        f"Environment {i} error, {e.__class__.__name__} {e}"
                    )
            ...
            if failed_count != 0:
                self._logger.warning(
                    f"{failed_count} environments failed. Restarting..."
                )
        ...
        ma_obs_list = {
            n: [np.concatenate(obs) for obs in zip(*ma_envs_obs_list[n])]
            for n in self.behavior_names
        }
        ...
        ma_max_step = {
            n: np.concatenate(ma_envs_max_step[n]) for n in self.behavior_names
        }
        ma_padding_mask = {
            n: np.concatenate(ma_envs_padding_mask[n]) for n in self.behavior_names
        }
...
if __name__ == "__main__":
    ...
    env = UnityWrapper(
        train_mode=True,
        env_name=r"D:\Code\PPO\UnityEnvs\maUGVRace",
        scene="UGVRace",
        no_graphics=False,
        n_envs=N_ENVS,
        group_aggregation=GROUP_AGGREGATION,
        ...
        for j in range(100):
            ...
            for n in ma_names:
                ...
                if ma_d_action_sizes[n]:
                    ...
                    d_action_list = [
                        np.random.randint(0, d_action_size, size=N_ENVS)
                        for d_action_size in d_action_sizes
                    ]
                    d_action_list = [
                        np.eye(d_action_size, dtype=np.int32)[d_action]
                        for d_action, d_action_size in zip(
                            d_action_list, d_action_sizes
                        )
                    ]
            ...
            ma_obs_list, ma_reward, ma_done, ma_max_step, ma_padding_mask = env.step(
                ma_d_action, ma_c_action
            )
