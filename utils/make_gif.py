...
def save_gif(env_name):
    ...
    print("total duration of gif : " +
          str(round(len(img_paths) * frame_duration / 1000, 2)) + " seconds")

    # save gif
    img, *imgs = [Image.open(f) for f in img_paths]
    img.save(fp=gif_path, format='GIF', append_images=imgs,
             save_all=True, optimize=True, duration=frame_duration, loop=0)
...
if __name__ == '__main__':
    ...
    save_gif_images(env_name, has_continuous_action_space,
                    max_ep_len, action_std)
