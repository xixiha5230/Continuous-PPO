...
def train():
    ...
    max_training_timesteps = int(1e7)  # 10M
    ...
    writer = tensorboardX.SummaryWriter(
        log_dir=f"{log_dir}/PPO_{env_name}_log_{run_num}")
    #####################################################
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len+1):
            ...
            if time_step % update_timestep == 0:
                actor_loss, critic_loss, dist_entropy, total_loss = ppo_agent.update()
                writer.add_scalar("Loss/actor", actor_loss, time_step)
                writer.add_scalar("Loss/critic", critic_loss, time_step)
                writer.add_scalar("Loss/total", total_loss, time_step)
                writer.add_scalar("entropy", dist_entropy, time_step)
