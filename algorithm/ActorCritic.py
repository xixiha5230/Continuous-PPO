...
class ActorCritic(nn.Module):
    ...
    def __init__(self, obs_space, action_space, config: ConfigHelper):
        ...
        in_features_size = self.obs_net.output_size if hasattr(self, 'obs_net') else obs_space.shape[0]
        ...
        after_rnn_size = self.rnn_net.output_size if hasattr(self, 'rnn_net') else in_features_size
    ...
    def forward(self, obs: torch.Tensor, hidden_in: torch.Tensor = None, sequence_length: int = 1, module_index: int = -1):
        '''
        Args:
    ...
    def eval_forward(self, obs: torch.Tensor, hidden_in: torch.Tensor = None, sequence_length: int = 1, module_index: int = -1):
        '''
