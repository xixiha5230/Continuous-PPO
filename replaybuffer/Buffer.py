...
class Buffer:
    ...
    def __init__(
        self,
        config: ConfigHelper,
        observation_space: gymnasium_spaces,
        action_space: gymnasium_spaces,
    ) -> None:
        """
        Args:
        ...
        if self.action_type == "continuous":
            self.actions = torch.zeros(
                (self.n_workers, self.worker_steps) + (action_space.shape[0],)
            ).to(self.device)
            self.log_probs = torch.zeros(
                (self.n_workers, self.worker_steps) + (action_space.shape[0],)
            ).to(self.device)
        elif self.action_type == "discrete":
            self.actions = torch.zeros((self.n_workers, self.worker_steps)).to(
                self.device
            )
            self.log_probs = torch.zeros((self.n_workers, self.worker_steps)).to(
                self.device
            )
        else:
        ...
        if isinstance(observation_space, (gym_spaces.Tuple, gymnasium_spaces.Tuple)):
            self.obs = [
                [
                    torch.zeros((self.n_workers,) + t.shape).to(self.device)
            ...
            self.obs = [
                [
                    torch.zeros((self.n_workers,) + observation_space.shape).to(
                        self.device
                    )
                ]
                for _ in range(self.worker_steps)
        ...
        # Advantage
        self.advantages = torch.zeros((self.n_workers, self.worker_steps)).to(
            self.device
        )
        ...
        if self.use_rnd:
            ...
            obs_0 = (
                observation_space[0]
                if isinstance(
                    observation_space, (gym_spaces.Tuple, gymnasium_spaces.Tuple)
                )
                else observation_space
            )
            self.rnd_next_obs = torch.zeros(
                (self.n_workers, self.worker_steps) + obs_0.shape
            ).to(self.device)
            self.rnd_values = torch.zeros((self.n_workers, self.worker_steps)).to(
                self.device
            )
            self.rnd_rewards = np.zeros(
                (self.n_workers, self.worker_steps), dtype=np.float32
            )
            self.rnd_advantages = torch.zeros((self.n_workers, self.worker_steps)).to(
                self.device
            )
    ...
    def prepare_batch_dict(self) -> None:
        ...
        samples = {
            ...
            "loss_mask": torch.ones(
                (self.n_workers, self.worker_steps), dtype=torch.bool
            ).to(self.device),
        }
        ...
        index = np.arange(0, self.n_workers * self.worker_steps).reshape(
            (self.n_workers, self.worker_steps)
        )
        self.flat_sequence_indices = np.asarray(
            self._arange_sequences(index, episode_done_indices)[0], dtype=object
        )
        ...
        for key, value in samples.items():
            ...
            sequences, max_sequence_length = self._arange_sequences(
            ...
            if isinstance(sequences[0], list):
                samples[key] = [
                    torch.stack(sequence_i, axis=0) for sequence_i in zip(*sequences)
                ]
            else:
        ...
        self.num_sequences = (
            len(sequences[0]) if isinstance(sequences[0], list) else len(sequences)
        )
    ...
    def _arange_sequences(self, data, episode_done_indices):
        ...
        for w in range(self.n_workers):
            ...
            for done_index in episode_done_indices[w]:
                ...
                episode_len = (
                    len(episode[0]) if isinstance(episode, list) else len(episode)
                )
                if self.sequence_length > 0:
                    for start in range(0, episode_len, self.sequence_length):
                        end = start + self.sequence_length
                        sequences.append(
                            [e[start:end] for e in episode]
                            if isinstance(episode, list)
                            else episode[start:end]
                        )
    ...
    def _pad_sequence(self, sequence: np.ndarray, target_length: int) -> np.ndarray:
        ...
        delta_length = target_length - len(
        ...
        if isinstance(sequence, list):
            ...
            for s in sequence:
                if len(s.shape) > 1:
                    ...
                    padding = torch.zeros(
                        ((delta_length,) + s.shape[1:]), dtype=s.dtype
                    ).to(self.device)
                else:
            ...
            if len(sequence.shape) > 1:
                ...
                padding = torch.zeros(delta_length, dtype=sequence.dtype).to(
                    self.device
                )
            # Concatenate the zeros to the sequence
    ...
    def recurrent_mini_batch_generator(self) -> dict:
        ...
        self.samples_flat["normalized_advantages"] = (
            self.samples_flat["advantages"] - self.samples_flat["advantages"].mean()
        ) / (self.samples_flat["advantages"].std() + 1e-8)
        if self.use_rnd:
            self.samples_flat["normalized_rnd_advantages"] = (
        ...
        for num_sequences in num_sequences_per_batch:
            ...
            for key, value in self.samples_flat.items():
                if key == "hxs" or key == "cxs":
                    ...
                    if isinstance(value, list):
                        mini_batch[key] = [
                            v[mini_batch_padded_indices].to(self.device) for v in value
                        ]
                    else:
                        mini_batch[key] = value[mini_batch_padded_indices].to(
                            self.device
                        )
            start = end
            yield mini_batch
    ...
    def calc_advantages(
        self, last_value: torch.Tensor, rnd_last_value: torch.Tensor = None
    ) -> None:
        ...
        with torch.no_grad():
            self._calc_advantages(
                self.dones, self.rewards, self.values, last_value, self.advantages
            )
            if self.use_rnd:
                self._calc_advantages(
    ...
    def _calc_advantages(
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(self.worker_steps)):
                ...
                delta = (
                    rewards[:, t] + self.gamma * last_value * mask[:, t] - values[:, t]
                )
